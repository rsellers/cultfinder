...
class EmotionalClassifier(BaseModel):
    ...
    context: str = Field(description="""A one-sentence description that explains the reason behind the numeric rating. If the materials are not relevant or complete enough to confidently answer the emotional questions, your best response is “the materials do not appear to be sufficient to provide a good answer”.""")
...
class EmotionalMetric(BaseModel):
    ...
    fairness: EmotionalClassifier = Field(description="""Is the token or project fair to the participants? 0 indicates widespread suspicion or distrust, 100 indicates the project or idea is widely accepted as being economically fair and evenly distributed.""")
    ...
    sell_intent: EmotionalClassifier = Field(description="""Are people openly selling tokens? 0 indicates everybody wants to sell the token, 100 indicates everybody wants to 'hold forever' or have 'diamond hands'.""")
    ...
    community_health: EmotionalClassifier = Field(description="""Does this appear to be a vibrant and lively discussion or is the community dead? 0 indicates an anemic 'dead' community with little interesting discussion, 100 indicates a vibrant healthy community with rich discussion.""")
...
class CommunityMetrics(BaseModel):
    ...
    about: str = Field(description="""Use a few words to describe what this community is all about and what bonds participants together.""")
